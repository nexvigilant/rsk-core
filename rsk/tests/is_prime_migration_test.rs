...
fn test_is_prime_full_migration() {
    ...
    let tree: DecisionTree = serde_yaml::from_str(&yaml_content).expect("Failed to parse DecisionTree");
    ...
    match engine.execute(&mut ctx) {
        ExecutionResult::Value(val) => {
            if let Value::Object(map) = val {
                assert_eq!(map.get("status"), Some(&Value::String("success".to_string())));
    ...
    match engine.execute(&mut ctx) {
        ExecutionResult::Value(val) => {
            if let Value::Object(map) = val {
                ...
                assert_eq!(map.get("message"), Some(&Value::String("Input 'n' is missing".to_string())));
            }
        },
