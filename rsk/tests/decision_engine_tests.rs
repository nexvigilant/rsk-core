use rsk::modules::decision_engine::{DecisionContext, DecisionEngine, DecisionNode, DecisionTree, ExecutionResult, Operator, Value};
...
fn test_complex_validation_logic() {
    ...
    let tree = DecisionTree {
        ...
        nodes: HashMap::from([
            ("check_username".to_string(), DecisionNode::Condition {
            ...
            ("check_age".to_string(), DecisionNode::Condition {
            ...
            ("check_email".to_string(), DecisionNode::Condition {
            ...
            ("pass".to_string(), DecisionNode::Return { value: Value::String("Valid".to_string()) }),
            ("fail_username".to_string(), DecisionNode::Return { value: Value::String("Invalid: Username missing".to_string()) }),
            ("fail_age".to_string(), DecisionNode::Return { value: Value::String("Invalid: Underage".to_string()) }),
            ("fail_email".to_string(), DecisionNode::Return { value: Value::String("Invalid: Bad email".to_string()) }),
        ])
    };
    ...
    assert_eq!(ctx.execution_path, vec!["check_username", "check_age", "check_email", "pass"]);
    ...
    assert_eq!(ctx.execution_path, vec!["check_username", "check_age", "fail_age"]);
    ...
    match engine.execute(&mut ctx) {
        ExecutionResult::Value(v) => assert_eq!(v, Value::String("Invalid: Username missing".to_string())),
        _ => panic!("Expected invalid result"),
    }
