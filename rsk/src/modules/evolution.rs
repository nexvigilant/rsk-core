...
pub fn synthesize_intrinsic(name: &str, tree: &DecisionTree) -> String {
    let fn_name = if name == "execute" { "execute".to_string() } else { "intrinsic_".to_string() + &to_snake_case(name) };
...
fn generate_value_code(val: &Value) -> String {
    match val {
        ...
        Value::String(s) => "Value::String(\"".to_string() + &s.replace('"', "\\\"") + "\".to_string())",
        Value::Array(arr) => {
...
mod tests {
    ...
    use std::collections::HashMap;
        ...
        fn test_synthesis_logic_flow() {
            ...
            nodes.insert("start".to_string(), DecisionNode::Condition {
            ...
            nodes.insert("high".to_string(), DecisionNode::Return { value: Value::String("high".to_string()) });

            nodes.insert("low".to_string(), DecisionNode::Return { value: Value::String("low".to_string()) });

            

            let tree = DecisionTree { start: "start".to_string(), nodes };
