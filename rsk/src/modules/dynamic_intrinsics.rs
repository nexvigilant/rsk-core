...
pub fn intrinsic_is_prime(input: Value) -> Value { 
    ...
    if let Value::Object(map) = input { variables = map; }
    ...
    if var_val == 0.0 {
        ...
        m.insert("message".to_string(), Value::String("Input 'n' must be an integer".to_string()));
        ...
        let mut m = HashMap::new();
        m.insert("status".to_string(), Value::String("error".to_string()));
        m.insert("message".to_string(), Value::String("Input 'n' is missing".to_string()));
        Value::Object(m)
    };
    }
}
