...
pub fn handle_json(action: &JsonAction) {
    match action {
        JsonAction::Parse { input } => {
            match json_processor::parse_json(input) {
                Ok(result) => println!("{}", json!(result)),
                Err(e) => println!("{}", json!({"status": "error", "message": e.to_string()})),
            }
        }
        JsonAction::Query { input, path } => {
            match json_processor::parse_json(input) {
                Ok(parsed) => {
        ...
        JsonAction::TypeCheck { input, expected } => {
            match json_processor::parse_json(input) {
                Ok(parsed) => {
                    let valid = json_processor::validate_type(&parsed.data, expected);
                    println!("{}", json!({"status": "success", "valid": valid, "expected": expected, "actual": parsed.value_type}));
                }
                Err(e) => println!("{}", json!({"status": "error", "message": e.to_string()})),
            }
        }
